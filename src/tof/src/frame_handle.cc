...
frame_t *handle_process(std::string s) {
  ...
  auto required_size = FRAME_HEAD_SIZE + FRAME_CHECKSUM_SIZE + FRAME_END_SIZE + 1;
...
__find_header:
  ...
  required_size = FRAME_HEAD_SIZE + frame_payload_len + FRAME_CHECKSUM_SIZE + FRAME_END_SIZE + 1;
  if (vecChar.size() < required_size) {
